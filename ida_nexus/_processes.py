...
def wait_process_exit(pid: int, timeout: float) -> bool:
    ...
    while True:
        try:
            # A terminated child remains visible to kill(pid, 0) as a zombie
            # until it is reaped. Detect that state without requiring psutil.
            waited, _status = os.waitpid(pid, os.WNOHANG)
            if waited == pid:
                return True
        except ChildProcessError:
            # The process is not our child; fall back to existence probing.
            pass
        try:
            os.kill(pid, 0)
        except ProcessLookupError:
